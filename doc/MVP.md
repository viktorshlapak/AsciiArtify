...
After successfully completing the PoC stage, the AsciiArtify team is ready to move to the MVP deployment stage.

The goal of this stage is to deploy a demo application to Kubernetes using ArgoCD and configure automatic synchronization from a Git repository.

ArgoCD is used as a GitOps tool that continuously tracks the desired application state in Git and applies it to the Kubernetes cluster.

## Source Repository

The MVP application is deployed from the following repository:

```text
https://github.com/den-vasyliev/go-demo-app
```

The repository contains a Helm chart located in the following directory:
...
The MVP was deployed using the following infrastructure:
...
An ArgoCD Application named `go-demo-app` was created to deploy the MVP.
...
The ArgoCD UI was available through the GitHub Codespaces forwarded port:

```text
https://organic-guacamole-4w979q4prwvfq7pp-18080.app.github.dev
```

During testing, the ArgoCD UI had a session issue through the Codespaces proxy. API authentication worked correctly, but UI requests to `/api/v1/applications` and `/api/v1/clusters` returned `401`.
...
The frontend service was exposed with:
...
The Ambassador service was exposed with:
...
The correct long-term GitOps solution is:

1. Fork the original repository.
2. Update the outdated Ambassador image in the Helm chart.
3. Point the ArgoCD Application to the forked repository.
...
[Watch MVP demo video](https://github.com/viktorshlapak/AsciiArtify/releases/tag/untagged-f68b589f94a1598c8f1b)

The demo shows:

1. ArgoCD UI with the `go-demo-app` application.
2. Source repository configuration.
3. Automated sync configuration.
4. Kubernetes resources in the `demo` namespace.
5. Running application through GitHub Codespaces forwarded port.
6. ArgoCD detecting drift and restoring the desired state from Git.
